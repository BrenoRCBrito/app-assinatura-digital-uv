...
function criarUsuarioDeTeste(id: UsuarioId, email: Email, senhaHash: string): Usuario {
  return { id, email, senhaHash, cpf: null, telefone: null, criadoEm: createIsoDateTime('2026-09-12T10:00:00.000Z') };
}
...
function mockUsuarios(usuarios: readonly Usuario[]) {
  ...
  jest.mocked(useRepositories).mockReturnValue({
    ...
    documentos: {} as never,
    usuarios: { findByEmail, findById: jest.fn(), create: jest.fn(), updateEmail: jest.fn(), 
      updateSenha: jest.fn(), updateCpf: jest.fn(), updateTelefone: jest.fn(), clear: jest.fn() },
